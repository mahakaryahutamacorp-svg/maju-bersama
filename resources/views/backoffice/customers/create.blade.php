...
<head>
    ...
    <title>Tambah Pelanggan | Maju Bersama ERP</title>
            ...
            <div class="flex items-center gap-3">
                <a href="/backoffice" class="block group">
                    ...
                    <div class="flex items-center gap-2">
                        <h1 class="text-base font-bold tracking-tight">Pendaftaran Pelanggan Baru</h1>
                    </div>
                </a>
            </div>
            <div class="flex items-center gap-3">
    ...
    <main class="flex-1 mx-auto w-full max-w-3xl p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
            <div class="border-b border-slate-100 pb-5 mb-6">
                <h2 class="text-lg font-bold text-slate-900">Formulir Data Pelanggan</h2>
                <p class="text-xs text-slate-500 mt-1">Masukkan biodata pelanggan dan tentukan grup harga (Pricing Tier) untuk harga khusus di POS Kasir.</p>
            </div>

            <form method="POST" action="{{ route('backoffice.customers.store') }}" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Contoh: Pak Suparman (Kelompok Tani Berkah)" required class="w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 focus:border-sky-500 focus:ring-1 focus:ring-sky-500 focus:outline-none">
                </div>
                ...
                <div class="rounded-xl border border-emerald-200 bg-emerald-50/50 p-4">
                    ...
                    <select id="customer_group_id" name="customer_group_id" class="w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2 text-sm font-semibold text-slate-900 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none">
                        ...
                        @foreach ($customerGroups as $cg)
                            <option value="{{ $cg->id }}" {{ old('customer_group_id') == $cg->id ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="phone" name="phone" value="{{ old('phone') }}" placeholder="08xxxxxxxxxx" class="w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2 text-sm text-slate-900 focus:border-sky-500 focus:ring-1 focus:ring-sky-500 focus:outline-none">
                    </div>
                    <div>
                        <label for="email" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Alamat Email</label>
                        <input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="pelanggan@example.com" class="w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2 text-sm text-slate-900 focus:border-sky-500 focus:ring-1 focus:ring-sky-500 focus:outline-none">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="address" name="address" rows="3" placeholder="Alamat rumah, desa, atau kelompok tani..." class="w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2 text-sm text-slate-900 focus:border-sky-500 focus:ring-1 focus:ring-sky-500 focus:outline-none">{{ old('address') }}</textarea>
                </div>
