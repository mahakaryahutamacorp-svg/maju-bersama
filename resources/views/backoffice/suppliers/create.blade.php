...
<head>
    ...
    <title>Tambah Supplier Baru | Maju Bersama ERP</title>
    ...
    <main class="mx-auto max-w-3xl px-6 py-8 lg:px-8">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-xs">
            <div class="border-b border-slate-200 px-6 py-5">
                <h2 class="text-lg font-bold text-slate-900">Tambah Supplier Baru</h2>
                <p class="mt-1 text-xs text-slate-500">
                    Daftarkan vendor atau penyedia barang untuk pembuatan Purchase Order (PO).
                </p>
            </div>

            <form method="POST" action="{{ route('backoffice.suppliers.store') }}" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" required placeholder="Contoh: PT Sumber Pangan Makmur" class="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                </div>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="text" id="contact_person" name="contact_person" value="{{ old('contact_person') }}" placeholder="Contoh: Bpk. Bambang" class="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label for="phone" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                            Nomor Telepon / WhatsApp
                        </label>
                        <input type="text" id="phone" name="phone" value="{{ old('phone') }}" placeholder="Contoh: 081234567890" class="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-sm font-mono focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="address" name="address" rows="3" placeholder="Alamat gudang / kantor supplier..." class="w-full rounded-xl border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">{{ old('address') }}</textarea>
                </div>

                <div class="flex items-center gap-3 pt-2">
                    <input type="hidden" name="is_active" value="0">
                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', '1') == '1' ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
