...
<head>
    ...
    <title>Laporan Pembelian &amp; PO | Maju Bersama ERP</title>
...
<body class="min-h-screen bg-slate-100 text-slate-900 antialiased flex flex-col">
    @php
        ...
        $purchases = $report['purchases'] ?? collect([]);
        $totalPurchases = $report['total_purchases'] ?? 0;
            ...
            <div class="flex items-center gap-3">
                <a href="/backoffice" class="block group">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="rounded bg-slate-800 px-1.5 py-0.5 text-[10px] font-mono text-slate-300">Purchase Report</span>
                    </div>
                </a>
            </div>
            <div class="flex items-center gap-3">
        ...
        <div class="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-3.5 sm:px-6 md:flex-row md:items-center md:justify-between lg:px-8">
            <div>
                <div class="flex items-center gap-2">
                    ...
                    <h2 class="text-lg font-bold text-slate-900">Laporan Pembelian &amp; PO</h2>
                </div>
                <p class="text-xs text-slate-500 mt-0.5">Daftar pesanan pembelian ke supplier, penerimaan barang, dan status pelunasan</p>
            </div>

            <!-- Form Filter Rentang Tanggal & Cabang -->
            <form method="GET" action="{{ route('reports.purchases') }}" class="flex flex-wrap items-center gap-2">
        ...
        <div class="print-container rounded-2xl border border-slate-300 bg-white shadow-lg p-6 sm:p-8 lg:p-10 text-slate-900">
            ...
            <header class="border-b-2 border-slate-900 pb-5 mb-6">
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    <div>
                        <p class="text-xs font-bold tracking-[0.2em] text-amber-600 uppercase">Sistem Pengadaan &amp; Hutang Dagang</p>
                    ...
                    <div class="sm:text-right">
                        ...
                        <p class="text-xs font-semibold text-slate-700 mt-1.5">Purchase Order &amp; Procurement</p>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                <div class="rounded-xl border border-slate-200 bg-slate-50/80 p-3.5">
                    <p class="text-[11px] font-semibold uppercase text-slate-500">Total Pesanan PO</p>
                    <p class="text-xl font-extrabold text-slate-900 mt-1">{{ number_format($totalOrders, 0, ',', '.') }}</p>
                    <p class="text-[10px] text-slate-400 mt-0.5">{{ number_format($totalItems, 0, ',', '.') }} unit barang</p>
                </div>

                <div class="rounded-xl border border-indigo-200 bg-indigo-50/50 p-3.5">
                    <p class="text-[11px] font-semibold uppercase text-indigo-800">Total Nilai Pembelian</p>
                    <p class="text-xl font-extrabold text-indigo-700 font-mono-num mt-1">{{ $formatRupiah($totalPurchases) }}</p>
                    <p class="text-[10px] text-indigo-600 mt-0.5">Nilai Pengadaan</p>
                </div>

                <div class="rounded-xl border border-emerald-200 bg-emerald-50/50 p-3.5">
                    <p class="text-[11px] font-semibold uppercase text-emerald-800">PO Selesai / Diterima</p>
                    <p class="text-lg font-bold text-emerald-700 font-mono-num mt-1">{{ number_format($completedOrders, 0, ',', '.') }}</p>
                    <p class="text-[10px] text-emerald-600 mt-0.5">Sudah Diterima</p>
                </div>

                <div class="rounded-xl border border-amber-200 bg-amber-50/50 p-3.5">
                    <p class="text-[11px] font-semibold uppercase text-amber-800">PO Pending / Proses</p>
                    <p class="text-lg font-bold text-amber-700 font-mono-num mt-1">{{ number_format($pendingOrders, 0, ',', '.') }}</p>
                    <p class="text-[10px] text-amber-600 mt-0.5">Menunggu Kiriman</p>
                </div>
            </div>
                ...
                <table class="w-full text-left text-xs border-collapse">
                    <thead>
                        <tr class="bg-slate-900 text-white uppercase text-[10px] tracking-wider">
                            ...
                            <th class="py-3 px-3 text-right">Total Beli</th>
                        ...
                        @forelse ($purchases as $po)
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="py-2.5 px-3 font-mono-num text-slate-700 whitespace-nowrap">
                                    {{ $po->order_date ? \Carbon\Carbon::parse($po->order_date)->isoFormat('DD/MM/YYYY') : \Carbon\Carbon::parse($po->created_at)->isoFormat('DD/MM/YYYY') }}
                                </td>
                                <td class="py-2.5 px-3 font-mono font-semibold text-slate-900 whitespace-nowrap">
                                    {{ $po->reference_number }}
                                </td>
                                <td class="py-2.5 px-3 text-slate-800">
                                    <div class="font-medium text-slate-900">{{ $po->supplier->name ?? 'Supplier Umum' }}</div>
                                    <div class="text-[10px] text-slate-400">Cabang: {{ $po->branch->name ?? '-' }} &bull; {{ $po->items->count() }} variasi produk</div>
                                </td>
                                <td class="py-2.5 px-3 text-right font-mono-num font-bold text-slate-900 whitespace-nowrap">
                                    {{ $formatRupiah((float) $po->total_amount) }}
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-8 text-center text-slate-400 italic">
                                    Tidak ada data pembelian pada periode dan kriteria cabang yang dipilih.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                        ...
                        <tfoot>
                            <tr class="bg-slate-100 font-bold border-t-2 border-slate-900 text-slate-900">
                                <td colspan="3" class="py-3 px-3 uppercase text-right tracking-wider">
                                    Total Nilai Pengadaan ({{ number_format($totalOrders, 0, ',', '.') }} Dokumen PO)
                                </td>
                                <td class="py-3 px-3 text-right font-mono-num text-sm text-indigo-800">
