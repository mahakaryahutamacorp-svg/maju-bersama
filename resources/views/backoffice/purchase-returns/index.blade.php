    ...
    <main x-data="purchaseReturnIndex()" class="mx-auto max-w-7xl px-6 py-8 lg:px-8 space-y-6">
        ...
        <div class="grid gap-4 sm:grid-cols-3">
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
                <p class="text-xs font-semibold uppercase tracking-wider text-slate-500">Total Nilai Retur</p>
                <p class="mt-2 text-2xl font-extrabold text-slate-950 font-mono">Rp {{ number_format($totalReturnsAmount, 0, ',', '.') }}</p>
                <p class="mt-1 text-xs text-slate-400">Total potongan hutang supplier</p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
                <p class="text-xs font-semibold uppercase tracking-wider text-slate-500">Jumlah Dokumen Retur</p>
                <p class="mt-2 text-2xl font-extrabold text-rose-600 font-mono">{{ $totalCount }}</p>
                <p class="mt-1 text-xs text-slate-400">Total transaksi retur terdaftar</p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs">
                <p class="text-xs font-semibold uppercase tracking-wider text-slate-500">Status Selesai</p>
                <p class="mt-2 text-2xl font-extrabold text-emerald-600 font-mono">{{ $completedCount }}</p>
                <p class="mt-1 text-xs text-slate-400">Telah dibukukan ke jurnal</p>
            </div>
        </div>
            ...
            <form method="GET" action="{{ route('backoffice.purchase-returns.index') }}" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-5 items-end">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if ($search || $startDate || $endDate || $status)
                        <a href="{{ route('backoffice.purchase-returns.index') }}" class="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition" title="Reset filter">
                            Reset
                        </a>
                    @endif
                </div>
                ...
                <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
                    <thead class="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-600">
                        <tr>
                            ...
                            <th class="py-3.5 px-4 w-36">Ref. Penerimaan</th>
                            <th class="py-3.5 px-4 text-right tabular-nums w-44">Total Rupiah</th>
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="py-3.5 px-4 whitespace-nowrap">
                                    <button
                                        ...
                                        title="Buka rincian transaksi retur ini"
                                    >
                                ...
                                <td class="py-3.5 px-4">
                                    <div class="font-bold text-slate-950">{{ $ret->supplier?->name ?? 'Supplier' }}</div>
                                    @if ($ret->supplier?->contact_person)
                                        <div class="text-[11px] text-slate-400">CP: {{ $ret->supplier->contact_person }}</div>
                                    @endif
                                </td>
                                <td class="py-3.5 px-4 whitespace-nowrap text-xs font-mono text-slate-600">
                                    @if ($ret->goodsReceipt)
                                        <button
                                            type="button"
                                            @click="loadTransaction('{{ $ret->goodsReceipt->reference_number }}')"
                                            class="text-teal-700 hover:underline hover:text-teal-900 font-semibold"
                                            title="Buka penerimaan barang terkait"
                                        >
                                            {{ $ret->goodsReceipt->reference_number }}
                                        </button>
                                    @else
                                ...
                                <td class="py-3.5 px-4 text-xs text-slate-600 max-w-xs truncate" title="{{ $ret->notes }}">
                                    {{ $ret->notes ?? '-' }}
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="py-12 text-center text-slate-400">
                                    ...
                                    <p class="mt-2 text-sm font-semibold text-slate-600">Belum ada riwayat retur pembelian</p>
                                    <p class="text-xs text-slate-400">Gunakan tombol "Input Retur Baru" untuk mencatat pengembalian barang.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
