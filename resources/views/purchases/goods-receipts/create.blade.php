...
<body class="min-h-screen bg-slate-100 text-slate-900 antialiased">
    ...
    <div class="border-b border-slate-200 bg-white">
        <div class="mx-auto flex max-w-7xl items-center justify-between overflow-x-auto px-6 py-2.5 lg:px-8">
            <div class="flex items-center gap-2 text-sm font-medium">
                ...
                <a href="/purchases/goods-receipts/create" class="rounded-lg bg-sky-600 px-3.5 py-2 font-semibold text-white shadow-sm">
                    + Penerimaan Baru
                </a>
            </div>
    ...
    <main class="mx-auto max-w-7xl space-y-6 px-6 py-8 lg:px-8">
        <!-- Error Banner -->
        @if ($errors->any())
            <div class="rounded-2xl border border-rose-200 bg-rose-50 p-5 text-rose-900 shadow-sm">
        ...
        <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div>
                <p class="text-sm font-semibold uppercase tracking-wider text-amber-600">Penerimaan Barang Masuk</p>
                <h2 class="mt-1 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">Formulir Goods Receipt</h2>
                <p class="mt-1 text-sm text-slate-500">
                    Masukkan rincian barang yang diterima dari supplier. Stok Gudang Pusat akan bertambah dan jurnal akuntansi ter-posting otomatis.
                </p>
            </div>
            <div>
        ...
        <!-- Form Alpine.js -->
        <div x-data="goodsReceiptForm(@js($products))" class="space-y-6">
            <form method="POST" action="/purchases/goods-receipts" id="receiptForm" class="space-y-6">
                @csrf

                <!-- Section 1: Informasi Nota Penerimaan -->
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div>
                            ...
                            <input type="date" name="date" value="{{ old('date', $todayDate) }}" required class="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="reference_number" value="{{ old('reference_number') }}" placeholder="Otomatis (atau isi manual)..." class="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500">
                        ...
                        <div>
                            ...
                            <input type="text" name="supplier_name" value="{{ old('supplier_name') }}" placeholder="Contoh: PT Sumber Makmur..." class="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="payment_type" x-model="paymentType" required class="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500">
                                <option value="cash">Tunai (Kas)</option>
                                <option value="credit">Tempo (Hutang Dagang)</option>
                            </select>
                            <span class="mt-1 block text-[11px] text-slate-400" x-text="paymentType === 'cash' ? 'Kredit ke Akun 1110 (Kas)' : 'Kredit ke Akun 2110 (Hutang Dagang)'"></span>
                        </div>
                        ...
                        <div class="sm:col-span-2 lg:col-span-4">
                            ...
                            <input type="text" name="notes" value="{{ old('notes') }}" placeholder="Catatan no PO, pengemudi, atau keterangan penerimaan..." class="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500">
                        </div>
                    </div>
                </section>
                ...
                <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div class="border-b border-slate-100 bg-slate-50/70 p-5 sm:flex sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p class="text-xs text-slate-500 mt-0.5">Pilih produk, kuantitas masuk, dan harga beli satuan.</p>
                        </div>
                        <div class="mt-3 sm:mt-0">
                            <!-- Tombol Tambah Baris -->
                            <button type="button" @click="addItem()" class="inline-flex items-center gap-1.5 rounded-xl bg-sky-600 px-4 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-sky-700">
                        ...
                        <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
                            <thead>
                                <tr class="border-b border-slate-200 text-xs font-bold uppercase tracking-wider text-slate-600">
                                    ...
                                    <th class="py-3 px-3 w-32 text-center">Kuantitas</th>
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <template x-for="(item, index) in items" :key="index">
                                    <tr class="transition hover:bg-slate-50/70">
                                        ...
                                        <td class="py-3 px-3">
                                            <select :name="'items[' + index + '][product_id]'" x-model="item.product_id" @change="onProductSelect(index)" required class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500">
                                                <option value="">-- Pilih Produk Dari Katalog --</option>
                                                <template x-for="p in products" :key="p.id">
                                                    <option :value="p.id" x-text="p.sku + ' - ' + p.name + ' (Stok: ' + p.stock + ')'"></option>
                                                </template>
                                            </select>
                                            <!-- Info stok saat ini -->
                                            <div x-show="item.product_id" class="mt-1 text-[11px] text-slate-500">
                                                Stok saat ini di Gudang Pusat: <span class="font-bold text-slate-800" x-text="getSelectedProductStock(item.product_id)"></span> unit
                                            </div>
                                        </td>

                                        <!-- Kuantitas -->
                                        <td class="py-3 px-3">
                                            <input type="number" :name="'items[' + index + '][quantity]'" x-model.number="item.quantity" @input="calculateSubtotal(index)" min="1" required class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-center text-sm font-bold font-mono text-slate-900 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500">
                                        </td>
                                        ...
                                        <td class="py-3 px-3">
                                            <div class="relative">
                                                ...
                                                <input type="number" :name="'items[' + index + '][unit_price]'" x-model.number="item.unit_price" @input="calculateSubtotal(index)" min="0" step="100" required class="w-full rounded-lg border border-slate-300 bg-white pl-9 pr-3 py-2 text-right text-sm font-mono font-semibold text-slate-900 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500">
                                            </div>
                                        </td>
                                        ...
                                        <td class="py-3 px-3 text-center">
                                            <button type="button" @click="removeItem(index)" :disabled="items.length <= 1" :class="items.length <= 1 ? 'opacity-30 cursor-not-allowed text-slate-400' : 'text-rose-500 hover:text-rose-700 hover:bg-rose-50'" class="rounded-lg p-1.5 transition" title="Hapus baris ini">
                    ...
                    <div class="border-t border-slate-200 bg-slate-50/80 p-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                        ...
                        <div class="flex items-center gap-4 text-right">
                            <span class="text-xs font-extrabold uppercase tracking-wider text-slate-500">Grand Total Pembelian:</span>
                ...
                <section class="rounded-2xl border border-sky-200 bg-sky-50/40 p-6 shadow-sm">
                    <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-sky-800 mt-0.5">
                                    Saat formulir disimpan, sistem akan langsung mencatat mutasi jurnal ganda (double-entry) berikut:
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-3 text-xs font-mono">
                            ...
                            <div class="rounded-xl border border-sky-300 bg-white p-2.5 text-left shadow-xs">
                                <span class="block text-[10px] font-bold text-sky-700 font-sans uppercase" x-text="paymentType === 'cash' ? 'Kredit (Kas 1110)' : 'Kredit (Hutang 2110)'"></span>
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    <a href="/purchases/goods-receipts" class="rounded-xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">
                        Batal
                    </a>
                    <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-7 py-3 text-sm font-bold text-white shadow-md transition hover:bg-emerald-700 focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2">
    ...
    <script>
        function goodsReceiptForm(productsList) {
            return {
