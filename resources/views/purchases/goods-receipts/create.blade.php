...
<body class="min-h-screen bg-slate-100 text-slate-900 antialiased">
    ...
    <main class="mx-auto max-w-7xl space-y-6 px-6 py-8 lg:px-8">
        ...
        <!-- Form Alpine.js -->
        <div x-data="goodsReceiptForm(@js($products), @js($activePOsData))" class="space-y-6">
            <form method="POST" action="/purchases/goods-receipts" id="receiptForm" class="space-y-6">
                ...
                <!-- Section 0: Tarik Data dari Purchase Order (Opsional) -->
                <section class="rounded-2xl border border-amber-300 bg-gradient-to-r from-amber-50 to-orange-50/40 p-6 shadow-xs">
                        ...
                        <div class="flex items-start gap-3.5">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-600 mt-0.5">
                                    Pilih PO aktif untuk mengisi otomatis daftar produk, sisa kuantitas belum diterima, dan harga beli kontrak PO.
                                </p>
                            </div>
                        </div>

                        <!-- Dropdown Searchable Select PO -->
                        <div class="w-full lg:w-96">
                            <label for="select_po" class="block text-[11px] font-bold uppercase tracking-wider text-slate-700 mb-1">
                                Pilih Dokumen PO (Pending / Partial):
                            </label>
                            <select
                                id="select_po"
                                x-model="selectedPoId"
                                @change="loadFromPO(selectedPoId)"
                                class="w-full rounded-xl border border-amber-300 bg-white px-3.5 py-2.5 text-sm font-semibold text-slate-900 shadow-xs outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-400"
                            >
                                <option value="">-- Penerimaan Mandiri (Tanpa PO) --</option>
                                <template x-for="po in activePOs" :key="po.id">
                                    <option :value="po.id" x-text="po.reference_number + ' | ' + po.supplier_name + ' (' + po.items.length + ' item sisa)'"></option>
                                </template>
                            </select>
                        </div>
                    </div>
                    ...
                    <template x-if="selectedPo">
                        <div class="mt-4 rounded-xl border border-amber-300 bg-white/80 p-3.5 flex flex-wrap items-center justify-between gap-3 text-xs">
                ...
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
                    ...
                    <div class="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="supplier_name" x-model="supplierName" placeholder="Contoh: PT Sumber Makmur..." class="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="payment_type" x-model="paymentType" :disabled="selectedPo !== null" required class="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:bg-slate-100 disabled:text-slate-500">
                                <option value="cash">Tunai (Kas)</option>
                                <option value="credit">Tempo (Hutang Dagang / Hutang Usaha)</option>
                            </select>
                        ...
                        <div class="sm:col-span-2 lg:col-span-4">
                            ...
                            <input type="text" name="notes" value="{{ old('notes') }}" placeholder="Catatan nomor truk, pengemudi, atau keterangan penerimaan..." class="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500">
                        </div>
                    </div>
                </section>
                    ...
                    <div class="border-b border-slate-100 bg-slate-50/70 p-5 sm:flex sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p class="text-xs text-slate-500 mt-0.5">
                                Kuantitas terisi dari sisa pesanan PO. Admin gudang dapat mengedit angka kuantitas sesuai barang aktual yang diterima.
                            </p>
                        </div>
    ...
    <script>
        function goodsReceiptForm(productsList, activePOsList) {
            return {
                products: productsList || [],
                activePOs: activePOsList || [],
                selectedPoId: '',
                selectedPo: null,
                supplierName: '',
                paymentType: 'cash',
                ...
                loadFromPO(poId) {
                    ...
                    // Bersihkan baris tabel yang ada dan isi dengan sisa barang dari PO
                    const newItems = [];
                    po.items.forEach(poItem => {
                        const remaining = parseInt(poItem.remaining_quantity) || 0;
                        if (remaining > 0) {
                            newItems.push({
                                product_id: poItem.product_id,
                                quantity: remaining,
                                unit_price: parseFloat(poItem.unit_price) || 0,
                                subtotal: remaining * (parseFloat(poItem.unit_price) || 0),
